...
- **레거시 워크스페이스 제자리 승격** — `va rebind`가 0.3.0 이전
  워크스페이스를 리비전 결박으로 백필한다(기본 dry-run). 원본이 사라진
  경우는 거부한다 — 동결됐던 코퍼스가 다시 새 증거를 받는다.
- **절차가 스킬 초안으로 컴파일** — 재등장하는 시각 실지지 `task-*`
  단계가 사람 승인 대기 스킬 초안이 되고, 도구 합의 취지 정합 게이트를
  거친다. `va skillgen --route`가 후보마다 남은 게이트를 말해준다.
- **비트 양자화 리듬 편집** — `va beats`가 음원에서 비트 그리드를
  추출하고 `va beat-eval`이 시퀀스 조인을 그 그리드에 게이트한다
  (p90 오프셋 <= 40ms). 스냅 제안은 원장을 건드리지 않는다.
- **스토리 맵** — 두 원장을 근거 교차 하이라이트·컷 출처 레인·화자
  리본과 함께 하나의 비례 타임라인에 투영한다.
- **영속 BM25 검색 인덱스** — 지문 기반 증분 FTS5 + 한국어 하이브리드
  회상 + RRF 라우팅 랭킹.
- **Windows 실험 지원** — 워크스페이스 락에 `msvcrt` 폴백이 들어가고
  CI가 Windows 스모크 잡을 돌린다; [요구 사항](#요구-사항) 참고.

전부 `main`에 반영됐다. 현재 릴리스는
[v0.4.0](https://github.com/mupozg823/timecode-agent/releases/tag/v0.4.0)이고,
